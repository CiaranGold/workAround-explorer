...
// Create input buttons for every company and role represented in salaryData
renderInputButtons(companies, 'company');
renderInputButtons(roles, 'role');

// Create a new <section> with radio inputs based on the data provided in the labels array
function renderInputButtons(labels, groupName) {
  ...
  labels.forEach(label => { // For each label...
    ...
    // Create a label for radio input element
...
function updateResults(){
  ...
  // If either the company or role is unselected, return nothing
  if (!company || !role) { return; }

  // WorkAroundModule functions calculate the needed data
